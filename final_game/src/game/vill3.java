 ...
 * in any city and play paper, scissors and rock and roll the dice.
 */
public class vill3 extends allVills{
	 ...
	 */
	vill3(){
		taunt = "<html>Cobra:<br/>I will kill you!<html>";
	}
	...
	public void villFight(Container container) {
		...
		JPanel GImage = new JPanel() {  
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill3.class.getResource("/res/battleBack.jpg"));  
		...
		JPanel hpIcon1 = new JPanel() {
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill3.class.getResource("/res/hpIcon.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
            icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		JPanel jpHero = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill3.class.getResource("/res/"+ hero.getName() +".png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
        icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		JPanel jpVill = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill3.class.getResource("/res/vill3.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
      icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		ImageIcon icon = new ImageIcon(vill3.class.getResource("/res/blood.gif"));
		...
		ImageIcon icon1 = new ImageIcon(vill3.class.getResource("/res/blood.gif"));
		...
		JPanel taunt = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill3.class.getResource("/res/taunt.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
           icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		talk.setText(vill3.taunt);
        talk.setFont(new Font(null,Font.BOLD,18));
        talk.setForeground(Color.WHITE);
        jptalk.add(talk);
		container.add(jptalk);
		
		new functionRollDiceVill3();
		functionRollDiceVill3.addRoll(container, hero, villHp, heroHp, 
