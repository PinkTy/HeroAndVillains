 ...
 * 
 * @author Cai & Lee
 * this class create a new villains, this villains only exist
 * in the last city and only play guess number.
 */
public class vill6 extends allVills{
	 ...
	 */
	vill6(){
		taunt = "<html>Big Boss:<br/>You will die!<html>";
	}
	...
	public void villFight(Container container) {
		...
		JPanel GImage = new JPanel() {  
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill6.class.getResource("/res/battleBack.jpg"));  
		...
		JPanel hpIcon1 = new JPanel() {
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill6.class.getResource("/res/hpIcon.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
            icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		JPanel jpHero = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill6.class.getResource("/res/"+ hero.getName() +".png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
        icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		JPanel jpVill = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill6.class.getResource("/res/vill6.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
      icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		ImageIcon icon = new ImageIcon(vill6.class.getResource("/res/blood.gif"));
		...
		ImageIcon icon1 = new ImageIcon(vill6.class.getResource("/res/blood.gif"));
		...
		JPanel taunt = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill6.class.getResource("/res/taunt.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
           icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		talk.setText(vill6.taunt);
        talk.setFont(new Font(null,Font.BOLD,18));
        talk.setForeground(Color.WHITE);
        jptalk.add(talk);
		container.add(jptalk);
		
		new functionGuessNumber().addGuss(container, hero, villHp, heroHp, 
				jpVill, jpHero, jpLairHP, jpHeroHP, hpIcon1, hpIcon2, lair.heroIndex,GImage1,GImage2,taunt,jptalk,lair.thisVill,lair.villIndex,GImage);
