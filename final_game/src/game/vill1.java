 ...
 * in the fist city and only play roll the dice.
 */
public class vill1 extends allVills{
	 ...
	 */
	vill1(){
		taunt = "<html>Naked Snake:<br/>You can't beat me<html>";
	}
	...
	public void villFight(Container container) {
		...
		JPanel GImage = new JPanel() {  
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill1.class.getResource("/res/battleBack.jpg"));  
		...
		JPanel hpIcon1 = new JPanel() {
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill1.class.getResource("/res/hpIcon.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
            icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		JPanel jpHero = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill1.class.getResource("/res/"+ hero.getName() +".png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
        icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		JPanel jpVill = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill1.class.getResource("/res/vill1.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
      icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		ImageIcon icon = new ImageIcon(vill1.class.getResource("/res/blood.gif"));
		...
		ImageIcon icon1 = new ImageIcon(vill1.class.getResource("/res/blood.gif"));
		...
		JPanel taunt = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill1.class.getResource("/res/taunt.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
           icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		talk.setText(vill1.taunt);
        talk.setFont(new Font(null,Font.BOLD,18));
        talk.setForeground(Color.WHITE);
        jptalk.add(talk);
		container.add(jptalk);
		new functionRollDice();
		functionRollDice.addRoll(container, hero, villHp, heroHp, 
