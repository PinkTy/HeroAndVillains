 ...
 * in the any city and only play paper, scissors and rock.
 */
public class vill2 extends allVills{
	 ...
	 */
	vill2(){
		taunt = "<html>Ocelot:<br/>Looser!!!!<html>";
	}
	...
	public void villFight(Container container) {
		...
		JPanel GImage = new JPanel() {  
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill2.class.getResource("/res/battleBack.jpg"));  
		...
		JPanel hpIcon1 = new JPanel() {
			 protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill2.class.getResource("/res/hpIcon.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
            icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		JPanel jpHero = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill2.class.getResource("/res/"+ hero.getName() +".png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
        icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		JPanel jpVill = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill2.class.getResource("/res/vill2.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
      icon.getIconHeight(), icon.getImageObserver()); 
			}
		...
		ImageIcon icon = new ImageIcon(vill2.class.getResource("/res/blood.gif"));
		...
		ImageIcon icon1 = new ImageIcon(vill2.class.getResource("/res/blood.gif"));
		...
		JPanel taunt = new JPanel() {
			protected void paintComponent(Graphics g) {  
				  ImageIcon icon = new ImageIcon(vill2.class.getResource("/res/taunt.png"));  
				  Image img = icon.getImage();  
				  g.drawImage(img, 0, 0, icon.getIconWidth(),  
           icon.getIconHeight(), icon.getImageObserver());  
			 }  
		...
		talk.setText(vill2.taunt);
        talk.setFont(new Font(null,Font.BOLD,18));
        talk.setForeground(Color.WHITE);
        jptalk.add(talk);
		container.add(jptalk);
		
		new functionPaperScissorsRock().addPaper(container, hero, villHp, heroHp, 
